#!/usr/bin/env bash
# Megatron + V1 async training for the blackbox jiuwenswarm recipe.
#
...
#
# which resolves each sample's task from task_config_jiuwenswarm.yaml
...
# jiuwenswarm knobs (run_timeout/conda_env) and the tool-image mount are
# configured in TASK_CONFIG (task_config_jiuwenswarm.yaml).
...
# in the Task Config (task_config_jiuwenswarm.yaml), not here.
