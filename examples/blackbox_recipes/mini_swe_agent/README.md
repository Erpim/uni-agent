...
**This recipe is a first-class `uni_agent` agent.** Unlike the old recipe (a
recipe-local `mini_swe_agent_runner.py` + custom `dataset.py`/`reward.py`), the
agent lives in `uni_agent/agents/mini_swe_agent/` and the sandbox/reward reuse
the framework's `openyuanrong` provider and `swe_rebench`/`swe_bench` tasks.
Training wires the unified runner bridge `uni_agent.framework.task_runner.run_task`,
which resolves each sample's task from `task_config_mini_swe_agent.yaml`
(agent + sandbox + sampling defaults) and deep-merges the runtime model binding.
...
### Task config (agent + sandbox + sampling)

Most recipe knobs moved into `task_config_mini_swe_agent.yaml`; edit it to tune
the agent without touching the training script:
...
| `agent.model.temperature/top_p/max_total_tokens` | `1.0/1.0/131072` | Policy sampling knobs |
...
| `GATEWAY_COUNT` | `1` | Gateway actors fronting the engine |
