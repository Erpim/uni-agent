...
# at /opt/mini-swe-agent) and talks to the policy gateway through a reverse
...
# (agent + sandbox + sampling defaults), deep-merges the sample values and the
# runtime model binding, and returns the reward via report_reward=True.
# The runner no longer carries tool_image / run_timeout / conda_env — those now
# live in the task_config YAML.
#
# Usage:
...
BY_PASS_MODE="${BY_PASS_MODE:-True}"        # rollout_correction.bypass_mode (老配置 True)
...
# mini-swe-agent step_limit/run_timeout/conda_env/tool-image mount now live in
# TASK_CONFIG (task_config_mini_swe_agent.yaml), not here.
TASK_CONFIG="${TASK_CONFIG:-${SCRIPT_DIR}/task_config_mini_swe_agent.yaml}"
TOOL_PARSER="${TOOL_PARSER:-qwen3_coder}"   # 网关 tool-call parser，须匹配模型 chat template
GATEWAY_COUNT="${GATEWAY_COUNT:-1}"
MAX_CONCURRENT_SESSIONS="${MAX_CONCURRENT_SESSIONS:-128}"
SERVED_MODEL_NAME="${SERVED_MODEL_NAME:-$(basename "${MODEL_PATH}")}"
# agent 已显式上报 finished（exit_status == "Submitted"）；置 True 将未完成
# episode 从 loss 中排除（配合 agent.py 里的 finished 字段）。
MASK_UNFINISHED_EPISODE="${MASK_UNFINISHED_EPISODE:-False}"
AGENT_LOG_DIR="${AGENT_LOG_DIR:-/tmp/uni_agent_logs}"
...
if ! timeout "${RAY_STATUS_TIMEOUT}" ray status &>/dev/null; then
    ...
    # NPU 环境的 head 资源；GPU 环境改为 --num-gpus="${TOTAL_GPUS}"
